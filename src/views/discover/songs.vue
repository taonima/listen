<template>
  <span>
    <Bubble ref="bubble">
      ...
      <template v-slot:content>
        <div class="bubble_content">
          ...
          <div v-for="(cat, k) in catlist" :key="k" class="filter_single">
            <div class="single_label"><Icon :name="cat.iconName"/><span class="name">{{cat.name}}</span></div>
            <div class="single_items">
            <div :class="single_item_class(item)" v-for="(item, k) in cat.sub" :key="k" @click="changeCat(item)">{{item.name}}</div>
            </div>
          </div>
        </div>
      </template>
    ...
    <div>
      <span>热门标签：</span>
      <span v-for="(tag, k) in tags" :key="k" class="hot_tag_name" @click="changeCat(tag)">{{tag.name}}</span>
    </div>
    <div class="playlists">
      <div v-for="(item, k) in playlists" :key="k" class="playlist_item" @click="goToSongSheet(item)">
